...
// 2. Write a JavaScript program to delete the rollno property from the following object. Also print the object before or after deleting the property. Sample object: 
var student = { name : "David Rayy", sclass : "VI", rollno : 12 };
...
var student = { name : "David Rayy", sclass : "VI", rollno : 12 }; 
console.log(Object.getOwnPropertyNames(student).length + Object.getOwnPropertySymbols(student).length);
...
{
    for(status in library[items])
        console.log(status, library[items][status]);
    console.log();
}
...
function cyliner(r, h)
{
    console.log(`Radius ${r} and height ${h}`);
    console.log((Math.PI * (r ** 2) * h).toFixed(4));
}
